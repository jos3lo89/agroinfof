...
import { ListarReuniones } from "../interfaces/interfaces";
...
const handleDeleteReu = async (id: string) => {
  try {
    ...
    if (confirm) {
      ...
      const res = await AxiosUser.borrarReuniones(id);
      console.log(res);
...
<template>
  <div class="text-center my-6">
    <h1 class="text-2xl capitalize font-semibold">ListaReuniones</h1>
  </div>
    ...
    >
      ...
      <tbody>
        ...
        >
          ...
          <td class="px-6 py-4">{{ reu.titulo }}</td>
          <!-- <td class="px-6 py-4">{{ formatFecha(reu.fecha_hora) }}</td> -->
          <td class="px-6 py-4">
            {{ new Date(reu.fecha_hora).toLocaleString() }}
          </td>
